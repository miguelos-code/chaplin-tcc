...
def get_request_role(request):
    """
    Obtém o role do usuário respeitando isolamento de abas.
    
    Verifica:
    1. Se existe tab_role_override (do middleware) - retorna isso
    2. Senão, retorna user.profile.role normalmente
    
    Args:
        request: HttpRequest
    
    Returns:
        str: Role (admin, gestor, lider, colaborador)
    """
    # Se há override de role da aba, usa
    if hasattr(request, 'tab_role_override') and request.tab_role_override:
        return request.tab_role_override
    
    # Senão, retorna role do usuário
